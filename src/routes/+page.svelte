...
<svelte:head>
	<meta
		name="description"
		content="natori - Backend Engineer. 大規模データ処理やWebアプリ開発に携わっています。"
	/>
</svelte:head>

<div class="rise flex flex-col items-center justify-center py-16 text-center">
	<div
		class="press mb-8 h-32 w-32 overflow-hidden rounded-full border-2 border-gray-200 shadow-sm transition-transform hover:scale-105 dark:border-gray-700"
	>
		<img src="/profile.png" alt={profile.name} class="h-full w-full object-cover" />
	</div>

	<h1 class="mb-2 font-sans text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white">
		@{profile.name}
	</h1>

	<p class="mb-6 font-medium text-gray-600 dark:text-gray-400">
		@{profile.title}
	</p>

	<div class="max-w-md">
		<p class="text-lg leading-relaxed whitespace-pre-line text-gray-900 italic dark:text-gray-300">
			"{profile.description}"
		</p>
	</div>
</div>
...
<section aria-labelledby="featured-heading" class="mb-12">
	...
	>
		<div class="mb-2 flex items-center gap-3">
			...
			<ArrowUpRight
				size={18}
				class="ml-auto text-gray-400 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
			/>
		</div>
